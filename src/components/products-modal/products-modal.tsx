...
export default function ProductsModal({ tab, setTab,categories, subCategories, isModalOpen, closeModal }: any) {
    ...
    const sendData = () => {
        if (tab == 'products') {
            ...
            ProductService.createProduct(name, description, ruName, ruDescription, category, subCategory, price, selectedFile).then(
                data => {
                    console.log(data);
                    closeModal();
            ...
            const ultimateErrors = {
                ...
                categoryRuName: categoryRuName.trim() ? '' : 'Iltimos ruscha nomini yozing',
            };

            setCategoryErrors(ultimateErrors);

            ProductService.createCategory(categoryName, categoryRuName).then(data => {
                console.log(data);
                closeModal();
                window.location.href = '/'
                setTab('categories')
            ...
            const ultimateErrors = {
                ...
                subCategoryRuName: subCategoryRuName.trim() ? '' : 'Iltimos ruscha nomini yozing',
                category: category.trim() ? '' : 'Kategoriya tanlang',
            };
            setSubCategoryErrors(ultimateErrors);

            ProductService.createSubCategory(subCategoryName, subCategoryRuName, category).then((data) => {
                console.log(data);
                closeModal();
                window.location.href = '/'
    ...
    if (tab == 'products') {
        ...
        return <Modal sendData={sendData} isOpen={isModalOpen} onClose={closeModal} title="Kategoriya qo'shish">
            <CategoriesForm errors={categoryErros} setCategoryName={setCategoryName} setCategoryRuName={setCategoryRuName} />
        </Modal>
    } else if (tab == 'sub-categories') {
        return <Modal sendData={sendData} isOpen={isModalOpen} onClose={closeModal} title="SubKategoriya qo'shish">
            <SubCategoriesForm errors={subCategoryErrors} categories={categories} setCategory={setCategory} setSubCategoryName={setSubCategoryName} setSubCategoryRuName={setSubCategoryRuName} />
        </Modal>
    }
}
